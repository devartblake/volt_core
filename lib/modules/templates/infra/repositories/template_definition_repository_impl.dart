...
class TemplateDefinitionRepositoryImpl implements TemplateDefinitionRepository {
  ...
  Future<List<FormTemplate>> listTemplates() async {
    ...
    if (remote != null) {
      try {
        ...
        // Offline fallback is derived from cached immutable definitions below.
      }
    }
  ...
  Future<FormTemplateDefinition?> getDefinition(
    ...
    if (remote != null) {
      try {
        ...
        // Use the exact cached revision below; never substitute another revision.
      }
    }
  ...
  String _cacheKey(FormTemplateDefinition definition) =>
      _cacheKeyFor(definition.template.id, definition.revision.id);
...
Map<String, dynamic> _definitionToCache(FormTemplateDefinition value) => {
      ...
      'options': value.options.map(formTemplateFieldOptionToSupabaseJson).toList(),
    };
