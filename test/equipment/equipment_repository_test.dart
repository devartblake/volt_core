...
void main() {
  ...
  group('EquipmentRepositoryImpl', () {
    ...
    test(
      'collapses repeat inspections of the same serial into one unit',
      () async {
    ...
    test('serial matching ignores case and surrounding whitespace', () async {
      await HiveBoxes.inspections.put(
        'i1',
        _inspection(id: 'i1', serial: 'cat-001 '),
      );
      await HiveBoxes.inspections.put(
        'i2',
        _inspection(id: 'i2', serial: ' CAT-001'),
      );
    ...
    test('blank nameplate fields fall back to the inspection', () async {
      await HiveBoxes.inspections.put(
        'i1',
        _inspection(id: 'i1', make: 'Kohler', voltage: '240V'),
      );
      await HiveBoxes.nameplates.put(
        'np1',
    ...
    test('status reflects the latest grade and deficiencies', () async {
      await HiveBoxes.inspections.put(
        'ok',
        _inspection(id: 'ok', serial: 'A', grade: 'Green'),
      );
      await HiveBoxes.inspections.put(
        'amber',
        _inspection(id: 'amber', serial: 'B', grade: 'Amber'),
      );
      await HiveBoxes.inspections.put(
        'red',
        _inspection(id: 'red', serial: 'C', grade: 'Red'),
      );
      await HiveBoxes.inspections.put(
        'def',
    ...
    test(
      'names a unit by make and model, falling back to site then address',
      () async {
        await HiveBoxes.inspections.put(
          'named',
    ...
    test('sorts by most recently inspected first', () async {
      ...
      expect((await repo.listEquipment()).map((e) => e.id).toList(), [
        'b',
        'a',
      ]);
    });
    ...
    test('facets skip blanks and de-duplicate', () async {
      await HiveBoxes.inspections.put(
        'i1',
        _inspection(id: 'i1', serial: 'A', make: 'Kohler'),
      );
      await HiveBoxes.inspections.put(
        'i2',
        _inspection(id: 'i2', serial: 'B', make: 'Kohler'),
      );
      await HiveBoxes.inspections.put(
        'i3',
        _inspection(id: 'i3', serial: 'C', make: ''),
      );
  ...
  group('EquipmentEntity.identityKey', () {
    ...
    test(
      'falls back to the inspection id when nothing identifies the unit',
      () {
        expect(
          EquipmentEntity.identityKey(
            serialNumber: '',
