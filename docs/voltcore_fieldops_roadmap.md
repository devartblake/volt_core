...
**Current Phase 3 slice:** published template definitions are read with the
authenticated Supabase client and cached in Hive as complete, revision-pinned
snapshots for offline rendering. An exact cached revision is used only when its
remote refresh fails; the repository never silently replaces a requested
revision with a newer one. A lifecycle service now supplies role-aware UI
affordances and clone/draft/publish/archive transition planning, including the
